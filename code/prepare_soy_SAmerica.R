...
extentsoy_resampled_output_name <- here("temp_data", "processed_soy_SAmerica", "southam_aoi", "soy_extent_resampledgaez_0120.tif")
unidirsoy_resampled_output_name <- here("temp_data", "processed_soy_SAmerica", "southam_aoi", "soy_unidir_resampledgaez_0220.tif")
#gaez_resampled_output_name <- here(gaez_dir, "southam_resampleddrivers_high_input.tif")
fc2k_resampled_output_name <- here("temp_data", "processed_fc2000", "southam_aoi", "fc_resampledgaez_2000.tif")
pst2k_resampled_output_name <- here("temp_data", "processed_pasture2000", "southam_aoi", "pasture_resampledgaez_2000.tif")

commodity_resampled_output_name <- here("temp_data", "processed_lossdrivers", "southam_aoi", "loss_commo_resampledgaez_0119.tif")

mask_path <- here("temp_data", "processed_lossdrivers", "southam_aoi", "always_zero_mask_loss_commo_resampledgaez_0119.tif")
...
aggr_output_name <- here("temp_data", "processed_soy_SAmerica", "southam_aoi", "soy_extent_9km_0120.tif")
...
# values are in hectares of soy in 3km x 3km grid cells - and THERE ARE NAs 
...
# define output file name
aggr_output_name <- here("temp_data", "processed_soy_SAmerica", "southam_aoi", "soy_unidir_9km_0220.tif")
...
fc2k_aggr_output_name <- here("temp_data", "processed_fc2000", "southam_aoi", "fc_9km_2000.tif")
...
### COMMODITY ###
# Input it from the tropical aoi preparation (prepare_loss_commodity.R)
...
# set na.rm = FALSE and run below until saveRDS to extract loss_commodity_aeay_long_allcells.Rdata
...
saveRDS(long_df, here("temp_data", "merged_datasets", "southam_aoi", "loss_commodity_aeay_long.Rdata"))

rm(long_df, varying_vars, gaez_southam, mask, losscommo, fc2k, pst2k)
...
path <- here("temp_data", "merged_datasets", "southam_aoi", "loss_commodity_aeay_long.Rdata")
df <- readRDS(path)
...
# saveRDS(df_cs, path)
saveRDS(df_cs, here("temp_data", "merged_datasets", "southam_aoi", "loss_commodity_aeay_cs_country_nf.Rdata"))
rm(df_cs)
...
df <- readRDS(here("temp_data", "merged_datasets", "southam_aoi", "loss_commodity_aeay_long.Rdata"))
...
saveRDS(remaining, here("temp_data", "merged_datasets", "southam_aoi", "loss_commodity_aeay_long_remaining.Rdata"))
...
saveRDS(df_cs, here("temp_data", "merged_datasets", "southam_aoi",  "loss_commodity_aeay_cs_stdeaear.Rdata"))

rm(df_cs)

#### MERGE ADDITIONAL VARIABLES ####  
# Base dataset (including outcome variable(s))
df_base <- readRDS(here("temp_data", "merged_datasets", "southam_aoi", "loss_commodity_aeay_long.Rdata"))


## COUNTRY
# just compute country and continent variables, even if invariant, so they can be called in generic function
df_country <- readRDS(here("temp_data", "merged_datasets", "southam_aoi", "loss_commodity_aeay_cs_country_nf.Rdata"))
...
## BIGGER CELLS
df_biggercells <- readRDS(here("temp_data", "merged_datasets", "southam_aoi", "loss_commodity_aeay_cs_biggercells.Rdata"))
...
## EAEAR
df_stdeaear <- readRDS(here("temp_data", "merged_datasets", "southam_aoi",  "loss_commodity_aeay_cs_stdeaear.Rdata"))  
