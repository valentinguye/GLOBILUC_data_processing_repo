...
datadir <- here("input_data", "GAEZ", "v4", "AEAY_bestoccuring", "Rain-fed-all-phases", "High-input")
targetdir <- here("temp_data", "GAEZ", "v4", "AEAY_bestoccuring", "Rain-fed-all-phases", "High-input")
...
for(crop in crops){
  ...
  crops[grepl(crop, crops)] <- mapmat[grepl(crop, paste0("yxHa_", mapmat[,"abrev"])),"Names"]
}
...
writeRaster(gaez_all, here("temp_data", "GAEZ", "v4", "AEAY_bestoccuring", "Rain-fed-all-phases", "high_input_all.tif"), 
            overwrite = TRUE)
