...
dir.create(here("temp_data", "processed_glass-glc", "test_aoi"), recursive = TRUE)
...
multiply <- raster("phtfLossSumGaez.tif")
multiply
plot(multiply)
summary(values(multiply))

gee <- raster("phtfLossSumGlass_maxpixels-0000000000-0000000000.tif")
gee
...
exrast <- parcels_brick[[12]]

unique(values(exrast))

# crop to a given TESTING AOI 

ext <- extent(110, 120, -3, 1)

test_aoi <- crop(parcels_brick, ext)

writeRaster(test_aoi, here("temp_data", "processed_glass-glc", "test_aoi", "brick_test_aoi.tif"))

##################
test_aoi <- brick( here("temp_data", "processed_glass-glc", "test_aoi", "brick_test_aoi.tif"))
...
for(t in 1:length(previous_years)){ # goes only up to 2014, as we don't need the average up to 2015.
  calc(test_aoi[[1:t]], fun = mean, 
           filename = here("temp_data", "processed_glass-glc", "test_aoi", paste0("past_mean_lu_",previous_years[t], ".tif")), 
...
rasterlist <- list.files(path = here("temp_data", "processed_glass-glc", "test_aoi"), 
...
for(t in 2:length(years)){ # starts from 1983 as we need t-1 and thus t starts from 2
  overlay(previous[[t-1]], test_aoi[[t]], fun = make_first_loss, 
          filename = here("temp_data", "processed_glass-glc", "test_aoi", paste0("first_loss_",years[t], ".tif")), 
          datatype = "INT1U", 
          overwrite = TRUE) 
}
...
for(t in 1:length(sbqt_years)){ 
  overlay(first_loss[[t]], test_aoi[[t+1]], # since test_aoi starts in 1982
          fun = make_sbqt_direct_lu, 
          filename = here("temp_data", "processed_glass-glc", "test_aoi", paste0("sbqt_direct_lu_",sbqt_years[t], ".tif")), 
          datatype = "INT1U", 
          overwrite = TRUE) 
}
...
for(t in 1:length(sbqt_years)){ # starts from 1983 as we need t-1 and thus t starts from 2
  calc(test_aoi[[t:length(sbqt_years)]], fun = getmode, 
          filename = here("temp_data", "processed_glass-glc", "test_aoi", paste0("future_mode_lu_",sbqt_years[t], ".tif")), 
          datatype = "INT1U", 
          overwrite = TRUE) 
}
# read it
...
for(t in 1:length(sbqt_years)){ 
  overlay(first_loss[[t]], mode_lu[[t]], 
          ...
          filename = here("temp_data", "processed_glass-glc", "test_aoi", paste0("sbqt_mode_lu_",sbqt_years[t], ".tif")), 
          datatype = "INT1U", 
          overwrite = TRUE) 
}
...
aggregate(first_loss, 
          ...
          expand = FALSE, 
          na.rm = FALSE, 
          filename = here("temp_data", "processed_glass-glc", "test_aoi", "aggr_first_loss.tif"),
          datatype = "INT1U",
          overwrite = TRUE)

aggr_first_loss <- brick(here("temp_data", "processed_glass-glc", "test_aoi", "aggr_first_loss.tif"))

# and now align precisely to GAEZ 

beginCluster() # this uses by default detectCores() - 1

resample(aggr_first_loss, gaez_aoi,
         method = "ngb",
         filename = here("temp_data", "processed_glass-glc", "test_aoi", "resampled_first_loss.tif"),
         datatype = "INT1U",
         overwrite = TRUE )
...
aggregate(sbqt_direct_lu, 
          ...
          expand = FALSE, 
          na.rm = FALSE, 
          filename = here("temp_data", "processed_glass-glc", "test_aoi", "aggr_sbqt_direct_lu.tif"),
          datatype = "INT1U",
          overwrite = TRUE)

aggr_sbqt_direct_lu <- brick(here("temp_data", "processed_glass-glc", "test_aoi", "aggr_sbqt_direct_lu.tif"))

# and now align precisely to GAEZ 

beginCluster() # this uses by default detectCores() - 1

resample(aggr_sbqt_direct_lu, gaez_aoi,
         method = "ngb",
         filename = here("temp_data", "processed_glass-glc", "test_aoi", "resampled_sbqt_direct_lu.tif"),
         datatype = "INT1U",
         overwrite = TRUE )
...
aggregate(sbqt_mode_lu, 
          ...
          expand = FALSE, 
          na.rm = FALSE, 
          filename = here("temp_data", "processed_glass-glc", "test_aoi", "aggr_sbqt_mode_lu.tif"),
          datatype = "INT1U",
          overwrite = TRUE)

aggr_sbqt_mode_lu <- brick(here("temp_data", "processed_glass-glc", "test_aoi", "aggr_sbqt_mode_lu.tif"))

# and now align precisely to GAEZ 

beginCluster() # this uses by default detectCores() - 1

resample(aggr_sbqt_mode_lu, gaez_aoi,
         method = "ngb",
         filename = here("temp_data", "processed_glass-glc", "test_aoi", "resampled_sbqt_mode_lu.tif"),
         datatype = "INT1U",
         overwrite = TRUE)
...
res_first_loss <- brick(here("temp_data", "processed_glass-glc", "test_aoi", "resampled_first_loss.tif"))
res_sbqt_direct_lu <- brick(here("temp_data", "processed_glass-glc", "test_aoi", "resampled_sbqt_direct_lu.tif"))
res_sbqt_mode_lu <- brick(here("temp_data", "processed_glass-glc", "test_aoi", "resampled_sbqt_mode_lu.tif"))
...
# proj <- projectRaster(dis, gaez_aoi, method = "ngb") # much less precise than aggregate_sbqt it seems
# 
# aligned <- resample(aggr, gaez_aoi, method = "ngb") # resample equivalent to projectRaster here.
# 
