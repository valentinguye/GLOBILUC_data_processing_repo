...
imf <- mutate(imf, POATS = as.numeric(POATS)/muv_index_2016, 
               POLVOIL = as.numeric(POLVOIL)/muv_index_2016, 
               PROIL = as.numeric(PROIL)/muv_index_2016, 
               PSUNO = as.numeric(PSUNO)/muv_index_2016)
class(imf[,2])
...
annual_imf <- ddply(imf, "year", summarise, 
                    price_oat = mean(POATS), 
                    price_olive_oil = mean(POLVOIL), 
                    price_rapeseed_oil = mean(PROIL), 
                    price_sunflower_oil = mean(PSUNO))
...
# Map Pink Sheet commodities with GAEZ crops
