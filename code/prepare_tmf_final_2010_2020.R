...
## THIS IS GAEZ IN FULL TROPICAL AOI 
# a priori no issue if it's gaez in global aoi, i.e. not croped in prepare_gaez.R, it only needs to be a larger aoi than continental ones given above. 
gaez <- stack(gaez_paths)

# save gaez crop names 
gaez_crops <- names(gaez)

### TMF DATA ###
# So out of GEE, TMF data is in 9 stacks, 3 continents * 3 drivers (plantation, flood, agri) called 'types' here, and each stack has 1990-2020 layers. 
# Resolution is 3km; 
...
### YEARS ### 
# ------------------------------------------ For this project, we need only years 2010-2020. -------------------------------------------------------
#----------------------------------- THIS IS THE MAIN DIFFERENCE WITH prepare_tmf_final_1990_2020.R script ---------------------------------------------- 
...
saveRDS(tropical_long_df, here("temp_data", "merged_datasets", "tmf_aoi", paste0("tmf_aeay_pantrop_long_",t0,"_",tT,".Rdata")))
...
r1 <- init(r, fun=sample(x=c(0,1, 0.5), replace = TRUE, size = 800))

r1_am <- crop(r1, ext_am)
# r1_af <- crop(r1, ext_af)
r1_as <- crop(r1, ext_as)
plot(r1_am)
# plot(r1_af)
...
plot(ext_af, add = T)
plot(ext_as, add = T)
plot(tmf_tropical_aoi, add = T)




template <- raster(tmf_tropical_aoi)
values(template) <- NA
proj4string(template) <- r1@crs

# writeRaster(template, file=here("temp_data", "processed_tmf", "tmf_tropical_aoi", "test_mosaic_template.tif"), format="GTiff", overwrite = TRUE)

writeRaster(r1_am, file=here("temp_data", "processed_tmf", "tmf_tropical_aoi", "r1_am.tif"), format="GTiff", overwrite = TRUE)
writeRaster(template, file=here("temp_data", "processed_tmf", "tmf_tropical_aoi", "r1_af.tif"), format="GTiff", overwrite = TRUE)
writeRaster(r1_as, file=here("temp_data", "processed_tmf", "tmf_tropical_aoi", "r1_as.tif"), format="GTiff", overwrite = TRUE)

# gather all AGRI rasters
agri_files <- c(here("temp_data", "processed_tmf", "tmf_tropical_aoi", "r1_af.tif"), 
                here("temp_data", "processed_tmf", "tmf_tropical_aoi", "r1_am.tif"), 
                here("temp_data", "processed_tmf", "tmf_tropical_aoi", "r1_as.tif"))

mosaic_rasters(gdalfile=agri_files ,dst_dataset=here("temp_data", "processed_tmf", "tmf_tropical_aoi", "test_mosaic_template.tif"), of="GTiff")

# filled template
ft <- raster(here("temp_data", "processed_tmf", "tmf_tropical_aoi", "test_mosaic_template.tif"))
plot(ft)
anyNA(values(ft))

tmf <- stack(here("temp_data", "processed_tmf", "tmf_tropical_aoi", paste0(type,"_3km.tif")))
vtmf <- values(tmf[[15]])
anyNA(vtmf)
plot(tmf[[15]])

agri_files <- c(here("input_data", "TMF", "TMF_defo_agri_3km_America.tif"), 
                here("input_data", "TMF", "TMF_defo_agri_3km_Africa.tif"), 
                here("input_data", "TMF", "TMF_defo_agri_3km_Asia.tif"))

am <- raster(agri_files[1])
af <- raster(agri_files[2])
as <- raster(agri_files[3])

plot(tmf_tropical_aoi)
plot(am, add = T)
plot(af, add = T)
plot(as, add = T)
...
#### OLD CODE ### 
## MOSAIC ## 
# using code from https://tengkengvang.com/2018/11/12/mosaic-or-merge-rasters-in-r/
# Create a template 

# set extent of template. It can be wider than all tiles going onto it
template <- raster(tmf_tropical_aoi)

# Note: this seems useless though. The extent of the mosaicked output from mosaic_rasters is the smallest that encompasses all tiles.   
# (as demonstrated by setting template extent to full tropical_aoi)
# it's actually used as background if it's one of the tiles, the first one specified in *_files

# set CRS of template
tmf_agri_am <- stack(here("input_data", "TMF", "TMF_defo_agri_3km_America.tif"))
proj4string(template) <- tmf_agri_am@crs

# set resolution of template 
res(template) <- res(tmf_agri_am)
# set values of template 
values(template) <- NA

# write template raster
writeRaster(template, file=here("temp_data", "processed_tmf", "tmf_tropical_aoi", "template_3km.tif"), format="GTiff", overwrite = TRUE)
...
agri <- mosaic_rasters(gdalfile=agri_files,
                       dst_dataset=here("temp_data", "processed_tmf", "tmf_tropical_aoi", "agri_3km.tif"), of="GTiff",
                       output.vrt = FALSE,
                       output_Raster = TRUE,
                       verbose = T)

gdalinfo(here("temp_data", "processed_tmf", "tmf_tropical_aoi", "agri_3km.tif"))

# gather all PLANTATION rasters
plantation_files <- c(here("temp_data", "processed_tmf", "tmf_tropical_aoi", "template_3km.tif"),
                      here("input_data", "TMF", "TMF_defo_plantation_3km_America.tif"), 
                      here("input_data", "TMF", "TMF_defo_plantation_3km_Africa.tif"), 
                      here("input_data", "TMF", "TMF_defo_plantation_3km_Asia.tif"))

mosaic_rasters(gdalfile=plantation_files ,dst_dataset=here("temp_data", "processed_tmf", "tmf_tropical_aoi", "plantation_3km.tif"), of="GTiff")
gdalinfo(here("temp_data", "processed_tmf", "tmf_tropical_aoi", "plantation_3km.tif"))

# gather all FLOOD rasters
flood_files <- c(here("temp_data", "processed_tmf", "tmf_tropical_aoi", "template_3km.tif"),
                 here("input_data", "TMF", "TMF_defo_flood_3km_America.tif"), 
                 here("input_data", "TMF", "TMF_defo_flood_3km_Africa.tif"), 
                 here("input_data", "TMF", "TMF_defo_flood_3km_Asia.tif"))

mosaic_rasters(gdalfile=flood_files ,dst_dataset=here("temp_data", "processed_tmf", "tmf_tropical_aoi", "flood_3km.tif"), of="GTiff")
gdalinfo(here("temp_data", "processed_tmf", "tmf_tropical_aoi", "flood_3km.tif"))
