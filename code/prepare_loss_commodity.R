...
rasterlist <- list.files(path = here("input_data", "10thLossSumGlass_maxP"), 
                         pattern = "^10thLoss_commodity_3km", 
                         full.names = TRUE) %>% as.list()
losscommo <- brick(rasterlist)
nlosslayers <- nlayers(losscommo)
# test <- losscommo[[1]]
# test
# plot(test)
# summary(values(test))

writeRaster(losscommo, here("temp_data", "processed_lossdrivers", "tropical_aoi", paste0("loss_commo_3km_01",nlosslayers, ".tif")), 
            overwrite = TRUE)

losscommo <- brick(here("temp_data", "processed_lossdrivers", "tropical_aoi", paste0("loss_commo_3km_01",nlosslayers, ".tif")))
...
losscommo_aggr_output_name <- here("temp_data", "processed_lossdrivers", "tropical_aoi", paste0("loss_commo_9km_01",nlosslayers, ".tif"))

# aggregate it from the 3km cells to 9km
raster::aggregate(losscommo, fact = 3,
                  expand = FALSE,
                  fun = sum,
                  ...
                  filename = losscommo_aggr_output_name,
                  # datatype = "INT2U", # let the data be float, as we have decimals in the amount of hectares. 
                  overwrite = TRUE)

# align to GAEZ exactly 
aggregated <- brick(losscommo_aggr_output_name)

losscommo_resampled_output_name <- here("temp_data", "processed_lossdrivers", "tropical_aoi", paste0("loss_commo_resampledgaez_01",nlosslayers, ".tif"))

resample(x = aggregated, 
         y = gaez, 
         method = "ngb", # summary exactly equal 
         filename = losscommo_resampled_output_name, 
         overwrite = TRUE)

# However, the resampling yields some (very few) negative values
losscommo <- brick(losscommo_resampled_output_name)
losscommo <- reclassify(losscommo, 
                        rcl = cbind(-Inf,0, 0), 
                        filename = losscommo_resampled_output_name, 
                        overwrite =TRUE) 
...
mask_path <- here("temp_data", "processed_lossdrivers", "tropical_aoi", paste0("always_zero_mask_loss_commo_resampledgaez_01",nlosslayers,".tif"))
...
tropical_stack <- stack(losscommo,
                        gaez, 
                        fc2k, 
                        pst2k)
# stock those names 
...
masked_stack_output_name <- here("temp_data", "merged_datasets", "tropical_aoi", paste0("loss_commo_masked_stack_01",nlosslayers,".tif"))
...
varying_vars <- list(names(losscommo))
...
saveRDS(long_df, here("temp_data", "merged_datasets", "tropical_aoi", "loss_commodity_aeayirr_long.Rdata"))
...
path <- here("temp_data", "merged_datasets", "tropical_aoi", "loss_commodity_aeayirr_long.Rdata")
df <- readRDS(path)
...
# saveRDS(df_cs, path)
saveRDS(df_cs, here("temp_data", "merged_datasets", "tropical_aoi", "loss_commodity_aeayirr_cs_country_nf.Rdata"))
rm(df_cs)
...
saveRDS(df_cs, here("temp_data", "merged_datasets", "tropical_aoi", "loss_commodity_aeayirr_cs_continent.Rdata"))
rm(df_cs)
...
grid_base <- raster(here("temp_data", "merged_datasets", "tropical_aoi", paste0("loss_commo_masked_stack_01",nlosslayers,".tif")))
...
saveRDS(df_cs, here("temp_data", "merged_datasets", "tropical_aoi", "loss_commodity_aeayirr_cs_biggercells.Rdata"))
...
df <- readRDS(here("temp_data", "merged_datasets", "tropical_aoi", "loss_commodity_aeayirr_long.Rdata"))
# Use cross section only
df_cs <- df[!duplicated(df$grid_id),]
...
saveRDS(df_cs, here("temp_data", "merged_datasets", "tropical_aoi", "loss_commodity_aeayirr_cs_stdeaear.Rdata"))  
rm(df_cs)
...
saveRDS(remaining, here("temp_data", "merged_datasets", "tropical_aoi", "loss_commodity_aeayirr_remaining.Rdata"))
...
df_base <- readRDS(here("temp_data", "merged_datasets", "tropical_aoi", "loss_commodity_aeayirr_long.Rdata"))

## COUNTRY
# just compute country and continent variables, even if invariant, so they can be called in generic function
df_country <- readRDS(here("temp_data", "merged_datasets", "tropical_aoi", "loss_commodity_aeayirr_cs_country_nf.Rdata"))
...
## CONTINENT
df_continent <- readRDS(here("temp_data", "merged_datasets", "tropical_aoi", "loss_commodity_aeayirr_cs_continent.Rdata"))
...
## BIGGER CELLS
df_biggercells <- readRDS(here("temp_data", "merged_datasets", "tropical_aoi", "loss_commodity_aeayirr_cs_biggercells.Rdata"))
...
## EAEAR
df_stdeaear <- readRDS(here("temp_data", "merged_datasets", "tropical_aoi",  "loss_commodity_aeayirr_cs_stdeaear.Rdata"))  
...
## REMAINING
#df_remain <- readRDS(here("temp_data", "merged_datasets", "tropical_aoi", "loss_commodity_aeayirr_long_remaining.Rdata"))
...
saveRDS(final, here("temp_data", "merged_datasets", "tropical_aoi", "loss_commodity_aeayirr_long_final.Rdata"))
