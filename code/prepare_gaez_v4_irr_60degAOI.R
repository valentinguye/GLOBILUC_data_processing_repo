...
dir.create(here("temp_data", "GAEZ", "v4", "AEAY_out_density", "Rain-fed-all-phases", "global_AOI", "High-input"), recursive = TRUE)
...
datadir <- here("input_data", "GAEZ", "v4", "AEAY_out_density", "Rain-fed-all-phases", "High-input")
targetdir <- here("temp_data", "GAEZ", "v4", "AEAY_out_density", "Rain-fed-all-phases", "global_AOI", "High-input")
...
for(crop in crops){
  ...
  crops[grepl(crop, crops)] <- mapmat[grepl(crop, paste0("ylHa_", mapmat[,"abrev"])),"Names"]
}

ext <- extent(c(-180, 179.9167, -60, 60))
...
writeRaster(gaez_all, here("temp_data", "GAEZ", "v4", "AEAY_out_density", "Rain-fed-all-phases", "global_AOI", "high_input_all.tif"), 
            overwrite = TRUE)
