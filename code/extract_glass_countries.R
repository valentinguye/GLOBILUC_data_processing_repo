...
# Write
writeRaster(parcels_brick, here("temp_data", "processed_glass-glc", "no_aoi", "brick_no_aoi.tif"), 
            overwrite = TRUE)


no_aoi <- brick(here("temp_data", "processed_glass-glc", "no_aoi", "brick_no_aoi.tif"))
...
for(t in 1:length(previous_years)){ # goes only up to 2014, as we don't need the average up to 2015.
  calc(no_aoi[[1:t]], fun = mean, 
       filename = here("temp_data", "processed_glass-glc", "no_aoi", paste0("past_mean_lu_",previous_years[t], ".tif")), 
...
rasterlist <- list.files(path = here("temp_data", "processed_glass-glc", "no_aoi"), 
...
for(t in 2:length(years)){ # starts from 1983 as we need t-1 and thus t starts from 2
  overlay(previous[[t-1]], no_aoi[[t]], fun = make_first_loss, 
          filename = here("temp_data", "processed_glass-glc", "no_aoi", paste0("first_loss_",years[t], ".tif")), 
          datatype = "INT1U", 
          overwrite = TRUE) 
}
...
for(t in 2:length(years)){ # starts from 1983 as we need t-1 and thus t starts from 2. And ends in 2014 as we need 2015 for sbqt. 
  overlay(previous[[t-1]], no_aoi[[t]], no_aoi[[t+1]], fun = make_nd_first_loss, 
          filename = here("temp_data", "processed_glass-glc", "no_aoi", paste0("nd_first_loss_",years[t], ".tif")), # nd for no disturbance
          datatype = "INT1U", 
          overwrite = TRUE) 
}

# loss no disturance: here we count forest loss as deforestation if there is a switch from forest to non forest, AND it remains not forest for at least a year. 
for(t in 2:length(years)){ # starts from 1983 as we need t-1 and thus t starts from 2. And ends in 2014 as we need 2015 for sbqt. 
  overlay(no_aoi[[t-1]], no_aoi[[t]], no_aoi[[t+1]], fun = make_nd_first_loss, 
          filename = here("temp_data", "processed_glass-glc", "no_aoi", paste0("nd_loss_",years[t], ".tif")), # nd for no disturbance
          datatype = "INT1U", 
          overwrite = TRUE) 
}
...
overlay(first_loss, cell_area, 
        fun = make_area, 
        filename = here("temp_data", "processed_glass-glc", "no_aoi", "ha_first_loss.tif"), 
        overwrite = TRUE)
...
overlay(nd_first_loss, cell_area, 
        fun = make_area, 
        filename = here("temp_data", "processed_glass-glc", "no_aoi", "ha_nd_first_loss.tif"), 
        overwrite = TRUE)

removeTmpFiles(h = 0)

## LOSS NO DISTRURBANCE 
rasterlist <- list.files(path = here("temp_data", "processed_glass-glc", "no_aoi"),
                         pattern = paste0("^nd_loss_"), 
                         full.names = TRUE) %>% as.list()
nd_loss <- stack(rasterlist)

cell_area <- area(nd_loss)

overlay(nd_loss, cell_area, 
        fun = make_area, 
        filename = here("temp_data", "processed_glass-glc", "no_aoi", "ha_nd_loss.tif"), 
        overwrite = TRUE)
...
first_loss_ha <- brick(here("temp_data", "processed_glass-glc", "no_aoi", "ha_first_loss.tif"))
...
nd_first_loss_ha <- brick(here("temp_data", "processed_glass-glc", "no_aoi", "ha_nd_first_loss.tif"))
