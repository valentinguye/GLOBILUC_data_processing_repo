...
# This script differs from prepare_loss_cropcommo_opindusnotrans.R in that 
# it counts cell area of loss-to-commodity cropland BY CROP
# by limiting the counting to cells where THAT CROP is the MAIN crop in cell in 2020, 
# according to CROPGRIDS. 

# For Curtis' commodity, we masked in GEE. But here, we need to agility to handle cropgrids data. 
# so before aggregating and resampling GEE_masked loss to gaez, I first mask it by cropgrids. 
...
irrigated_crops <- c("Barley", "Buckwheat", "Cabbage", "Carrot", "Chickpea", # Cassava,
...
# so finally, rainfed crops comprise perennials (incl. cassava), as well as soybean. 
...
# NEW STEP: mask it at CROPGRIDS resolution (~5km) 
...
transition_types <- c("cultpasture", "cocoa", "croplandcommo", "oilpalmindus_notrans")#
# and add to the loop the outcomes masked to CROPGRIDS. 
...
for(type in c(transition_types, paste0(analysis_crop_groups,"_CGweighted"))){
...
resampled_ouput_nameS <- here("temp_data", "processed_lossdrivers", "tropical_aoi", 
                              paste0("loss_",c(transition_types, paste0(analysis_crop_groups,"_CGweighted")),"_resampledgaez_01",nlosslayers, ".tif"))
names(resampled_ouput_nameS) <- c(transition_types, paste0(analysis_crop_groups,"_CGweighted"))
...
# DO include cocoa in the mask, otherwise many cells where this is the only driver of deforestation would
# be masked out as having always 0 defo in the other types. 
# Note that adding pastures from GPW reduces greatly the mask, because there 
# are pastures (even just cultivated) in many places that croplands does not detect. 
# As a result, the final data is much (~x3) bigger.  
any_type <- stack(resampled_ouput_nameS[c("croplandcommo", "oilpalmindus_notrans", "cultpasture", "cocoa")]) # # select here the LUC to use in the mask

always_zero <- function(y){if_else(condition = (sum(y)==0), true = 0, false = 1)}

mask_path <- here("temp_data", "processed_lossdrivers", "tropical_aoi", paste0("always_zero_mask_loss_cropcommo_opindusnotrans_cocoa_cultpasture_anytype_resampledgaez_01",nlosslayers,".tif"))
...
#### 2000 ACCESSIBILITY #### 
# downloaded there https://forobs.jrc.ec.europa.eu/products/gam/download.php
# pre-aggregated in GEE to 9km 
# values already transformed in GEE from minutes to hours (to nearest 50k people city)
# access <- raster(here("input_data", "acc_50k_2000_9km.tif"))
# # plot(access)
# 
# #### RESAMPLE #### 
# summary(values(access))
# # there are NAs, because we did not aggregate in R with na.rm argument (just ike for pst2k)
# # but they are only in the sea. Things on land, but places very close to a city have low but still positive values
# min(values(access), na.rm=T)
# access <- reclassify(access, cbind(NA, 0))
# 
# resample(x = access, 
#          y = gaez, 
#          method = "ngb", # summary exactly equal 
#          filename = here("temp_data", "acc_50k_2000_resampledgaez.tif"), 
#          overwrite = TRUE)
...
#lossoilpalmboth <- brick(resampled_ouput_nameS["oilpalmbothcommo"])
lossoilpalmindus <- brick(resampled_ouput_nameS["oilpalmindus_notrans"])
losscocoa <- brick(resampled_ouput_nameS["cocoa"])
losspasture <- brick(resampled_ouput_nameS["cultpasture"])

lossmaize <- brick(resampled_ouput_nameS["Maizegrain_CGweighted"])
losscereals <- brick(resampled_ouput_nameS["Cereals_CGweighted"])
lossrice <- brick(resampled_ouput_nameS["Rice_CGweighted"])
lossroots <- brick(resampled_ouput_nameS["Roots_CGweighted"])
losssoy <- brick(resampled_ouput_nameS["Soy_compo_CGweighted"])
lossoilfeed <- brick(resampled_ouput_nameS["Oilfeed_crops_CGweighted"])
losscotton <- brick(resampled_ouput_nameS["Cotton_CGweighted"])
lossbiomass <- brick(resampled_ouput_nameS["Biomass_CGweighted"])
losssugarcane <- brick(resampled_ouput_nameS["Sugarcane_CGweighted"])
losstobacco <- brick(resampled_ouput_nameS["Tobacco_CGweighted"])
# losscitrus <- brick(resampled_ouput_nameS["Citrus_CGweighted"])
...
names(losscocoa) <- paste0("loss_cocoa.",seq(2001, 2019, 1)) 
names(losspasture) <- paste0("loss_pasture.",seq(2001, 2019, 1)) 
# Make it easy here, to match those outcomes with their respective crop group later 

names(lossmaize) <- paste0("loss_Maizegrain.",seq(2001, 2019, 1)) 
names(losscereals) <- paste0("loss_Cereals.",seq(2001, 2019, 1)) 
names(lossrice) <- paste0("loss_Rice.",seq(2001, 2019, 1)) 
names(lossroots) <- paste0("loss_Roots.",seq(2001, 2019, 1)) 
names(losssoy) <- paste0("loss_Soy_compo.",seq(2001, 2019, 1)) 
names(lossoilfeed) <- paste0("loss_Oilfeed_crops.",seq(2001, 2019, 1)) 
names(losscotton) <- paste0("loss_Cotton.",seq(2001, 2019, 1)) 
names(lossbiomass) <- paste0("loss_Biomass.",seq(2001, 2019, 1)) 
names(losssugarcane) <- paste0("loss_Sugarcane.",seq(2001, 2019, 1)) 
names(losstobacco) <- paste0("loss_Tobacco.",seq(2001, 2019, 1)) 
...
tropical_stack <- stack(losscropland, lossoilpalmindus, # lossoilpalmboth, # 
                        losscocoa, losspasture,
                        lossmaize,
                        losscereals,
                        lossrice,
                        lossroots,
...
# They should not have NAs before they are masked 
# if(tropical_stack %>% values() %>% anyNA()){stop("unexpected to have NAs here before adding NAs with the masking below.")}
# The above is quite long, so check manually some of those rasters
# losscotton$loss_Cotton.2001 %>% values() %>% summary()
# lossmaize$loss_Maizegrain.2001 %>% values() %>% summary()
# losscereals$loss_Cereals.2001 %>% values() %>% summary()
# losspasture$loss_pasture.2001 %>% values() %>% summary()
# losscocoa$loss_cocoa.2001 %>% values() %>% summary()
# fc2k %>% values() %>% summary
# pst2k %>% values() %>% summary
# access %>% values() %>% summary

mask <- raster(mask_path)
if(mask %>% values() %>% anyNA()){stop("unexpected to have NAs here, the mask function is not designed for this")}

masked_stack_output_name <- here("temp_data", "merged_datasets", "tropical_aoi", paste0("loss_cropcommo_CGweighted_opindusnotrans_cultpasture_anytype_masked_stack_01",nlosslayers,".tif"))
...
# Note that it removes cells (rows) as soon as one raster layer has NA for this pixel (not only if all layers are NA)
...
varying_vars <- list(names(losscropland), 
                     names(lossoilpalmindus),  # names(lossoilpalmboth),
                     names(losscocoa), 
                     names(losspasture),
                     names(lossmaize),
                     names(losscereals),
...
saveRDS(long_df, here("temp_data", "merged_datasets", "tropical_aoi", "loss_cropcommo_CGweighted_opindusnotrans_cultpasture_cocoa_aeaycompo_long.Rdata"))
...
path <- here("temp_data", "merged_datasets", "tropical_aoi", "loss_cropcommo_CGweighted_opindusnotrans_cultpasture_cocoa_aeaycompo_long.Rdata")
df <- readRDS(path)
...
# saveRDS(df_cs, path)
saveRDS(df_cs, here("temp_data", "merged_datasets", "tropical_aoi", "loss_cropcommo_CGweighted_opindusnotrans_cultpasture_cocoa_aeaycompo_cs_country_nf.Rdata"))
rm(df_cs)
...
saveRDS(df_cs, here("temp_data", "merged_datasets", "tropical_aoi", "loss_cropcommo_CGweighted_opindusnotrans_cultpasture_cocoa_aeaycompo_cs_continent.Rdata"))
rm(df_cs)
...
grid_base <- raster(here("temp_data", "merged_datasets", "tropical_aoi", paste0("loss_cropcommo_CGweighted_opindusnotrans_cultpasture_anytype_masked_stack_01",nlosslayers,".tif")))
...
saveRDS(df_cs, here("temp_data", "merged_datasets", "tropical_aoi", "loss_cropcommo_CGweighted_opindusnotrans_cultpasture_cocoa_aeaycompo_cs_biggercells.Rdata"))
...
df <- readRDS(here("temp_data", "merged_datasets", "tropical_aoi", "loss_cropcommo_CGweighted_opindusnotrans_cultpasture_cocoa_aeaycompo_long.Rdata"))
# Use cross section only
df_cs <- df[!duplicated(df$grid_id),]
...
# # Check which crops are the most often the max in their categories
# df_cs1 <- dplyr::mutate(df_cs, across(.cols = all_of(c("Cassava", "Sweetpotato", "Whitepotato","Yam")),
#                                       .fns = ~.==Roots,
#                                       .names = paste0("{.col}","_ismax_of_roots"))) 
# 
# summary(df_cs1$Cassava_ismax_of_roots)
# summary(df_cs1$Sweetpotato_ismax_of_roots)
# summary(df_cs1$Whitepotato_ismax_of_roots)
# summary(df_cs1$Yam_ismax_of_roots)
...
saveRDS(df_cs, here("temp_data", "merged_datasets", "tropical_aoi", "loss_cropcommo_CGweighted_opindusnotrans_cultpasture_cocoa_aeaycompo_cs_stdeaear.Rdata"))  
rm(df_cs)
...
# 
# saveRDS(remaining, here("temp_data", "merged_datasets", "tropical_aoi", "loss_cropcommo_CGweighted_opindusnotrans_cultpasture_cocoa_aeaycompo_remaining.Rdata"))
# 
...
df_base <- readRDS(here("temp_data", "merged_datasets", "tropical_aoi", "loss_cropcommo_CGweighted_opindusnotrans_cultpasture_cocoa_aeaycompo_long.Rdata"))

## COUNTRY
# just compute country and continent variables, even if invariant, so they can be called in generic function
df_country <- readRDS(here("temp_data", "merged_datasets", "tropical_aoi", "loss_cropcommo_CGweighted_opindusnotrans_cultpasture_cocoa_aeaycompo_cs_country_nf.Rdata"))
...
## CONTINENT
df_continent <- readRDS(here("temp_data", "merged_datasets", "tropical_aoi", "loss_cropcommo_CGweighted_opindusnotrans_cultpasture_cocoa_aeaycompo_cs_continent.Rdata"))
...
## BIGGER CELLS
df_biggercells <- readRDS(here("temp_data", "merged_datasets", "tropical_aoi", "loss_cropcommo_CGweighted_opindusnotrans_cultpasture_cocoa_aeaycompo_cs_biggercells.Rdata"))
...
## EAEAR
df_stdeaear <- readRDS(here("temp_data", "merged_datasets", "tropical_aoi",  "loss_cropcommo_CGweighted_opindusnotrans_cultpasture_cocoa_aeaycompo_cs_stdeaear.Rdata"))  
...
## REMAINING
#df_remain <- readRDS(here("temp_data", "merged_datasets", "tropical_aoi", "loss_cropcommo_CGweighted_opindusnotrans_cultpasture_cocoa_aeaycompo_long_remaining.Rdata"))
...
saveRDS(final, here("temp_data", "merged_datasets", "tropical_aoi", "loss_cropcommo_CGweighted_opindusnotrans_cultpasture_cocoa_aeaycompo_long_final.Rdata"))
